...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- To ask for that input, create an interaction on the current issue with POST /api/issues/{issueId}/interactions using kind suggest_tasks, ask_user_questions, or request_confirmation. Use continuationPolicy wake_assignee when you need to resume after either acceptance or rejection; use wake_assignee_on_accept only when rejection needs no follow-up.",
  ...
  "- For plan approval, update the plan document first, then create request_confirmation targeting the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId}. Wait for acceptance before creating implementation subtasks. If the confirmation is rejected, fetch the latest interaction result.reason, revise the plan document, and create a fresh confirmation.",
...
export function renderPaperclipWakePrompt(
  ...
  if (normalized.interactionKind || normalized.interactionStatus) {
    lines.push(`- interaction: ${normalized.interactionKind ?? "unknown"} (${normalized.interactionStatus ?? "unknown"})`);
  }
  if (normalized.issue?.workMode === "planning") {
    ...
    const rejectedPlanContinuation =
      !hasWakeComments &&
      normalized.interactionKind === "request_confirmation" && normalized.interactionStatus === "rejected";
    ...
    if (rejectedPlanContinuation) {
      directive = "Revise the rejected plan only. Do not create child issues, write code, or perform implementation work.";
    }
    ...
    if (rejectedPlanContinuation) {
      lines.push(
        "- rejected-plan continuation: fetch /api/issues/{issueId}/interactions, read the latest request_confirmation result.reason, update the plan document to address it, and create a fresh request_confirmation",
      );
    }
  }
